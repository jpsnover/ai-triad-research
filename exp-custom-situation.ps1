...
$ResearchDir = (Resolve-Path (Join-Path $PSScriptRoot 'research/comp-linguist')).Path
$env:AI_USAGES_PATH = Join-Path $ResearchDir 'ai-usages-local.json'
Write-Host "Set AI_USAGES_PATH to: $env:AI_USAGES_PATH" -ForegroundColor DarkCyan
...
$RepoRoot = $RepoRoot = $PSScriptRoot
...
foreach ($t in $Topics) {
    ...
    $cfg = [ordered]@{
        ...
        slug=$slug; temperature=0.3; topic=$t.text;  stageModels=@{
               cite = $CiteModel
                # Add this new entry for your custom situation context model
                situation_context = @{ # You might need to confirm the exact stage name used by cli.ts
                    usageId = 'debate.custom-situation-context'
                    values = @{
                        node_id = '$t.id' # Pass the topic ID as the node_id
                        speaker_pov = '$cfg.activePovers' # Pass all active POVs; custom model will iterate/select
                       bdi_category = 'all' # Custom model will iterate/select BDI categories
                       corpus_path = (Join-Path $ResearchDir 'situation_statements_corpus.json')
                   }
               }
           }
    }
