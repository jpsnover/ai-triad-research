...
"@
                        ...
                        try {
                            ...
                            foreach ($Key in @('GEMINI_API_KEY', 'ANTHROPIC_API_KEY', 'GROQ_API_KEY', 'AI_MODEL')) {
                                ...
                                if (-not [string]::IsNullOrWhiteSpace($Val)) {
                                    # Write both sh and pwsh formats
    ...
    if (Test-Path $EnvFilePath) {
        ...
        Write-Host ''
        Write-Info 'To load these settings in future sessions, add to your shell profile:'
        Write-Host ''

        $ShellProfile = if ($PROFILE) { $PROFILE } else { '~/.config/powershell/Microsoft.PowerShell_profile.ps1' }

        Write-Host "  # PowerShell ($ShellProfile):" -ForegroundColor Gray
        Write-Host "  Get-Content ~/.aitriad-env | ForEach-Object {" -ForegroundColor White
        Write-Host "      if (`$_ -match '^export (\w+)=`"(.+)`"`$') {" -ForegroundColor White
        Write-Host "          [Environment]::SetEnvironmentVariable(`$Matches[1], `$Matches[2])" -ForegroundColor White
        Write-Host '      }' -ForegroundColor White
        Write-Host '  }' -ForegroundColor White
        Write-Host ''
        Write-Host '  # bash/zsh (~/.bashrc or ~/.zshrc):' -ForegroundColor Gray
        Write-Host '  source ~/.aitriad-env' -ForegroundColor White
        Write-Host ''
    }
}
