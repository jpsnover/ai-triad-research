...
The application communicates with its server via HTTP (REST) through a centralized bridge layer (`web-bridge.ts`). In web deployment on Azure Container Apps, the server may be cold-starting, overloaded, or temporarily unreachable. Client code must handle these conditions gracefully — never silently swallowing errors, never retrying unsafe mutations, and never leaving the user staring at a spinner with no feedback.

### **All Fetch Calls Must Route Through web-bridge.ts**

The bridge (`src/renderer/bridge/web-bridge.ts`) centralizes timeout, retry, error normalization, and 429/503 handling. Bare `fetch()` or `new XMLHttpRequest()` calls in renderer code bypass these protections.

**Severity: Medium — Consistency / Reliability**

The reviewer must grep renderer files for direct `fetch(` calls outside of the bridge module itself. Approved exceptions (documented in each file):
- `analyticsEmitter.ts` — fire-and-forget telemetry via `sendBeacon`
- `flightRecorderInit.ts` — flight recorder upload (must work even when bridge is broken)
- `trace.ts` — low-level performance tracing
- `dumpToast.ts` — error dump upload

Any other bare `fetch()` in `src/renderer/` is a defect. The fix is always to use the bridge's `get()`, `post()`, `put()`, or `del()` helpers.

```typescript
// BAD — bypasses timeout, retry, and error normalization
const res = await fetch('/api/taxonomy/nodes');
const data = await res.json();

// GOOD — uses bridge with centralized error handling
import { get } from '../bridge/web-bridge';
const data = await get('/api/taxonomy/nodes');
```

### **Every HTTP Request Must Have a Timeout**

A request without a timeout blocks the UI indefinitely when the server hangs. The bridge's `get()` function must enforce a default timeout (30s for reads). Mutation endpoints (`post`, `put`, `del`) should use appropriate timeouts based on expected duration (e.g., 180s for AI calls, 30s for data saves).

**Severity: High — Availability**

The reviewer must verify:
1. The bridge's `get()` function uses `AbortController` with a timeout (not just `signal` passthrough)
2. Long-running endpoints (AI generation, batch operations) have explicit extended timeouts documented in the call site
3. Timeout errors surface to the user as actionable messages, not silent failures
...
// GOOD — timeout via AbortController with user-visible error
async function get(path: string, opts?: { timeoutMs?: number }) {
...
### **Silent Mutation Error Swallowing**

When a save, update, or delete fails, the user must know. Silent swallowing of mutation errors causes data loss — the user believes their edit was saved when it was not.

**Severity: High — Data Integrity**

The reviewer must check every `post()`, `put()`, and `del()` call site for:
1. A `catch` block that shows user-visible feedback (toast, inline error, modal)
2. Flight recorder logging in the `catch` (`getGlobalRecorder()?.record(...)`)
3. No bare `catch {}` or `catch { console.error(...) }` without user notification

```typescript
// BAD — user thinks the save worked
try {
  await post('/api/taxonomy/nodes', { data: node });
} catch {
  console.error('save failed');
}

// GOOD — user sees the failure and can retry
try {
  await post('/api/taxonomy/nodes', { data: node });
} catch (err) {
  getGlobalRecorder()?.record({
    type: 'system.error', component: 'node-editor',
    level: 'error', message: 'Failed to save node',
    error: { name: (err as Error).name, message: String(err) },
  });
  toast.error('Failed to save node. Check your connection and try again.');
}
```

### **Retry Safety — Never Auto-Retry Non-Idempotent Mutations**

Retrying a `POST` that creates a resource can produce duplicates. Retrying a `PUT` that increments a counter double-counts. Only idempotent reads (`GET`) and explicitly idempotent mutations (replace semantics, delete by ID) are safe to auto-retry.

**Severity: High — Data Integrity**

The reviewer must verify:
1. Automatic retry logic (if present) is restricted to `GET` requests and responses with `429` or `503` status codes
2. `POST`/`PUT`/`DELETE` failures surface to the user for manual retry — the user decides, not the code
3. If a mutation endpoint is genuinely idempotent (e.g., `PUT /api/settings` with full-replace semantics), this is documented in a comment at the retry site
4. `429 Retry-After` headers are respected — the retry delay uses the server's value, not a hardcoded backoff

```typescript
// BAD — blindly retries a POST that creates a debate
async function createDebate(topic: string) {
  return retryWithBackoff(() => post('/api/debates', { topic }));
}

// GOOD — only retries safe reads; mutations surface errors to user
async function createDebate(topic: string) {
  return post('/api/debates', { topic });
  // Caller handles error with toast + manual retry button
}
```

### **User Feedback on Network Failure**

When a network error occurs (server unreachable, DNS failure, TLS error), the user must see a meaningful message — not a raw exception, a blank screen, or a perpetual loading spinner.

**Severity: Low — UX Quality**

The reviewer should check that:
1. Network errors produce user-readable messages (not raw `TypeError: Failed to fetch`)
2. Loading states have timeout fallbacks — a spinner that displays for >30s should show "Still loading... the server may be slow" or similar
3. Components that depend on server data render a fallback state (empty state, cached data, or error boundary) rather than crashing on fetch failure
