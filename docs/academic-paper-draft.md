...
AI policy discourse involves competing empirical claims, normative commitments, and strategic reasoning that existing NLP tools compress into lossy single-label representations. I present an integrated system for multi-perspective discourse analysis built on a **neural-symbolic architecture** that pairs LLM-based content generation with symbolic validation, formal computation, and deterministic explanation. The system combines three layers: (1) **ontological grounding** using DOLCE D&S for perspectival multiplicity, BDI (Belief-Desire-Intention) for agent characterization, and AIF for argumentation structure — adopted as vocabulary in JSON structures rather than formal OWL/RDF reasoning; (2) **formal argumentation** via Quantitative Bipolar Argumentation Frameworks (QBAFs) with DF-QuAD gradual semantics and a novel BDI-aware base score calibration; and (3) **confidence-gated iterative extraction** (FIRE) that replaces single-shot summarization with a per-claim verification loop. The system organizes AI policy literature through a 1,182-node taxonomy spanning four perspectives (accelerationist, safetyist, skeptic, and shared situations), supports multi-agent debates with ontology-grounded context injection, deploys an active moderator with a 14-move intervention taxonomy, and feeds debate findings back into the taxonomy through concession harvesting and post-debate reflections. All convergence diagnostics and debate outcome explanations are computed deterministically from the argument network without LLM calls. BDI-aware calibration reveals a fundamental asymmetry: AI reliably scores strategic claims (Intentions: r=0.71, ICC 0.70, α 0.68) but not normative (Desires: r=0.65 is a rank artifact unsupported by absolute-agreement metrics) or empirical (Beliefs) claims — an asymmetry deeper than the self-contained versus externally-verifiable distinction initially hypothesized. Additional contributions include a 5-mechanism topic-alignment pipeline that enforces debate scope through defense in depth, a 9-category AI voice tell taxonomy with per-character de-artifacting that achieves 80-93% voice tell reduction (model-dependent), and epistemic type integration connecting six claim types to debate rhetoric, moderation, and validation. The system is evaluated across 100+ debates with empirical findings on relevance threshold calibration, embedding model selection, and per-claim sycophancy detection.
...
2. **Formal argumentation with BDI-aware scoring.** A QBAF implementation using DF-QuAD gradual semantics (Rago et al., 2016) with a novel BDI-aware base score calibration. Through iterative calibration (Q-0), I discovered that different BDI layers require fundamentally different scoring approaches: strategic claims (Intentions) can be reliably AI-scored via decomposed evidence criteria, while normative claims (Desires) show rank correlation without absolute agreement, and empirical claims (Beliefs) require human judgment — a finding that extends the self-contained vs. externally-verifiable distinction to reveal that self-containment is necessary but not sufficient for reliable automated scoring.
...
9. **Topic-alignment enforcement.** A 5-mechanism pipeline (TopicScope extraction, prompt boundary placement, draft quality gate, taxonomy constraint-aware filter, moderator drift detection) that enforces debate scope through defense in depth across extraction, prompt construction, validation, taxonomy selection, and moderation stages — each degrading gracefully when scope data is unavailable.

10. **Voice differentiation (de-artifacting).** A 9-category taxonomy of AI voice tells (hedge stacking, false balance, formulaic transitions, bullet-point reasoning, sycophantic framing, meta-commentary, synonym chains, theatrical emphasis, passive voice overuse) with per-character voice specifications that achieve 80-93% reduction in voice tell frequency on Claude Opus 4.6 — a strongly model-dependent finding with implications for multi-model agent deployments.

11. **Epistemic type integration.** Six epistemic types (strategic_recommendation, empirical_claim, interpretive_lens, predictive, normative_prescription, definitional) integrated into five debate engine components — debater context, moderator drift detection, plan stage strategy, audience-weighted prioritization, and draft quality gate calibration — connecting the ontological structure of the taxonomy to the rhetorical demands of each claim type.

12. **Affect-aware debate diagnostics.** A five-dimension emotional-register model (urgency, fear, hope, outrage, empathy) scored per turn from curated lexicons via inflection-aware word-boundary matching, yielding an affect-intensity measure and a phase-appropriateness score that compares each turn's register against phase-specific baselines. This operationalizes Wachsmuth et al.'s (2017) emotional-appeal dimension as a deterministic, auditable calibration metric.
...
Abstract argumentation frameworks (Dung, 1995) compute argument acceptability from attack relationships. Bipolar frameworks (Cayrol and Lagasquie-Schiex, 2005) add support relationships. Quantitative extensions assign numerical strength to arguments: QBAFs (Baroni et al., 2019) combine base scores with gradual semantics to propagate strength through attack/support networks. DF-QuAD (Rago et al., 2016) provides a specific gradual semantics that is discontinuity-free and guarantees convergence. Gould and Toni (2025) extend this line to a learned setting: their Gradual AA-CBR builds an edge-weighted QBAF over training cases (each case an argument for its own label, attacking opposing-label cases and supporting agreeing ones) and resolves it with a modular, MLP-simulating gradual semantics (Potyka, 2021) whose differentiability lets base scores and edge weights be learned by gradient descent. Two features of their construction inform ours. First, their final acceptability map σ: A → [0,1] ∪ {⊥} carries an explicit undefined state ⊥, the formal analogue of the censored / not-yet-decidable outcome our convergence reporting must keep separate from a decision rather than fold into one. Second, they replace the single default argument of classic AA-CBR — which structurally biases the outcome toward one label — with a *set* of target cases, one per outcome, a neutrality property directly relevant to a multi-camp design in which no perspective should be the structural default.
...
**Consensus versus disagreement as the objective.** The closest published analog to our runtime loop is Du et al. (2024), whose multiagent debate shares our loop's shape — multiple rounds, cross-visibility of prior responses, mutual revision — while inverting its objective. Their loop drives *homogeneous* instances of a single model toward a *common answer* graded against ground truth (arithmetic, GSM8K, MMLU, biography factuality); ours drives *heterogeneous fixed personas* (accelerationist, safetyist, skeptic) to surface and preserve disagreement, graded on process rather than on any answer. Forced consensus is damage in our setting: a three-camp debate that collapses to a single shared answer has failed at its job of showing the reader the structure of a genuine disagreement, and our convergence diagnostic exists partly to *detect* premature collapse rather than to maximize agreement. This inversion makes "consensus improves factuality" incoherent as our frame — the claim presupposes the ground truth a contested-values domain lacks, and importing accuracy-style scoring would require inventing a correct answer to a values disagreement. Du et al. thereby throws two of our contributions into relief: ontology-grounded situation injection as a controlled treatment on debate substance, and process-level telemetry (crux engagement, repetition, claim retention, convergence trajectory, situation uptake — instruments still under validation) as the outcome measure, occupy exactly the region their homogeneous, consensus-seeking, ground-truth-graded design does not.
...
**AIF (argumentation vocabulary).** Eight canonical, AIF-aligned edge types formalize relationships between taxonomy nodes: SUPPORTS, CONTRADICTS, ASSUMES, WEAKENS, RESPONDS_TO, TENSION_WITH, INTERPRETS, and CONVERGES_WITH — the last linking a POV node to a shared consensus situation node where perspectives agree. Attack types follow Pollock's tripartite classification: *rebut* (contradicts the conclusion), *undercut* (denies the inference), and *undermine* (attacks premise credibility). CONVERGES_WITH is defined in the ontology and wired into the scoring layer; population of consensus edges via concession-harvesting is ongoing.

### 3.2 Taxonomy Structure

The taxonomy contains 1,182 nodes organized into four POVs:
...
### 3.5 Structured Proposition Layer

The system derives a structured proposition layer over extracted claims to support consistency checking and cross-camp comparison. Each claim is formalized as a neo-Davidsonian event frame with a predicate over an explicit event variable, participants bound to a DOLCE-typed entity register, and thematic roles drawn from a closed vocabulary. Attitudes (belief, desire, intention) are attributed to a camp and reified in first-order form through a `holds/3` relation rather than modal operators, and the framework deliberately omits a belief-closure axiom, so a camp's commitments are not treated as logically omniscient. Topical grounding that fills no predicate role is carried in a separate `about[]` projection. The layer is produced by a language-model formalization pass and is not trusted until measured. A stratified golden set scores each frame per component (predicate, arguments, polarity, modality, temporal, grounding), yielding a `formalization_accuracy` metric. On a 45-claim stratified golden set scored against a single-annotator reference, the pass attains a strict claim-level formalization accuracy of 0.778 (0.978 when minor, non-meaning-changing deviations are permitted), with predicate selection on multi-clause, meta-descriptive claims the principal residual error. The lenient figure is a diagnostic upper bound, not the headline accuracy. Consistent with the project's vocabulary-over-formalism commitment, all formal machinery lives in regenerable sidecar artifacts, and the source claims remain human-readable prose with typed references.

*Caveats:* agreement is measured against a single-annotator reference; `match_level` is `exact`-only in practice because the resolver hardcodes it, so subclass/superclass grounding is not yet exercised on real data.
...
**Edge attribution.** To support explainability beyond dialectic traces (Section 6.13), the engine provides removal-based edge attribution (`computeEdgeAttribution`): for a target node, it computes the DF-QuAD strength with the full graph, then recomputes with each incident edge removed, measuring the strength delta. The result is a per-edge attribution score quantifying each edge's contribution to the node's final strength — positive for edges that strengthen the node, negative for edges that weaken it. This computation is cheap (<30ms for 30 edges in practice) because each removal requires only local re-propagation rather than full-graph recomputation, and it provides actionable diagnostic information: "this node is strong primarily because of support edge X, and would drop below 0.5 if attack edge Y were removed." This removal-and-recompute procedure is an **interventional (ablation) diagnostic**: each edge removal is an intervention on the argument graph, and the resulting strength delta is that edge's causal contribution to the node's acceptability. Jin et al. (2026, ARGORA) reach the same remove-and-recompute diagnostic from the opposite direction — explicitly casting the argument graph as a causal model — corroborating that formal argument strengths admit causal-style attribution without importing an external structural causal model.
...
**Results.** Both Desires and Intentions were AI-scored (Beliefs human-scored); the agreement statistics below show the AI scoring validated only for Intentions:

| Metric | Desires (n=8) | Intentions (n=9) | Aggregate |
|--------|---------|------------|-----------|
| Pearson r (calibration) | 0.65 | 0.71 | — |
| ICC(2,1) | 0.09 | 0.70 | — |
| Krippendorff α (interval) | −0.18 | 0.68 | — |
| MAE (hold-out) | 0.13 | 0.24 | 0.19 |

Reporting Pearson r alone overstated Desires calibration. Under absolute-agreement statistics (ICC(2,1) and Krippendorff's α, interval), only **Intentions** shows genuine rater agreement (ICC 0.70, α 0.68); the Desires correlation (r=0.65) is not supported by agreement metrics (ICC 0.09, α −0.18) — the model assigns a near-constant score across Desires, so its rank correlation reflects a flat predictor rather than discriminating power. We therefore treat Intentions as the sole AI-calibrated layer and defer Desires alongside Beliefs pending the evidence pipeline. Intentions MAE (0.24) is at the threshold — acceptable but flagged for monitoring in production use. Per-category sample sizes are small (Desires n=8, Intentions n=9); these results should be treated as directional.
...
The synthesis also produces an AIF-aligned argument map: claims with IDs, near-verbatim text, speaker attribution, and typed relationships (supported_by with scheme, attacked_by with attack_type, argumentation_scheme, and critical_question_addressed). An explicit neutralization pass strips speaker-attributed framing from extracted agreement/disagreement points before argument mapping.
...
A structural risk in multi-agent debate is *persona contamination*: the evaluator inherits framing from the agents it judges. If synthesis knows that "Accelerationist" represents accelerationism, it may unconsciously weigh arguments through that lens rather than assessing reasoning quality neutrally.
...
8. **Camp Insularity** — the degree to which a POV engages only its own taxonomy and prior claims rather than cross-camp material. When the insularity rate exceeds threshold with sufficient sample, the moderator injects a cross-camp taxonomy node to force cross-perspective engagement.

Each identified crux carries a counterfactual-type discriminator — *interventional* (what if we changed X?), *backtracking* (what if X had been otherwise?), *normative* (what if the value ordering differed?), or *none* for non-counterfactual cruxes. The system logs the distribution of these types per debate, separating disagreements that are empirically resolvable (interventional/backtracking) from those rooted in value conflict (normative). This treats each crux as an intervention on the argument network — remove the flipping argument, recompute the outcome — a causal, remove-and-recompute view of decisive arguments that Jin et al. (2026, ARGORA) independently validate.

These eight signals are displayed together in the diagnostics panel as a per-turn convergence dashboard. Because all signals are computed from the argument network's graph structure and node metadata (QBAF strengths, edge types, move classifications), they are fully deterministic — running the same computation on the same argument network always produces identical results. This makes convergence assessment debuggable: if a diagnostic flags an anomaly, the underlying graph data can be inspected directly.

**Process reward scores.** In addition to the eight convergence diagnostics, each turn receives a continuous process reward score that composes five dimensions: *engagement* (whether the turn responds to the specific claim it was directed to address), *novelty* (inverse of recycling against prior same-speaker turns), *consistency* (alignment with the speaker's commitment store), *grounding* (proportion of claims tied to taxonomy nodes or cited evidence), and *move quality* (appropriateness of the selected dialectical move given the current phase and argument network state). The composite score provides a per-step quality signal analogous to process reward models (Lightman et al., 2023), enabling fine-grained identification of where debate quality degrades — at the individual turn level rather than only at the debate level. Unlike outcome reward (which evaluates only the final synthesis), process reward scores identify *which turns* contributed to or detracted from convergence, informing both real-time moderator steering and post-hoc analysis of debate dynamics.
...
Persona-aware modifiers implement a learn-from-experience loop. Each debater-move pair carries a prior probability reflecting persona-specific expectations — for instance, the accelerationist persona (Accelerationist) has elevated priors for PIN and PROBE, reflecting a tendency to resist commitment. These priors decay toward the neutral baseline (1.0) as the moderator actually fires the corresponding move, using exponential decay at rate 0.15 per observed trigger. The effect is that the moderator's expectations converge on observed behavior rather than stereotypes: a debater who responds well to early PINs will face fewer subsequent PINs as the prior decays.
...
Multi-agent debates exhibit a scope management challenge absent in single-turn tasks: without structural enforcement, debaters drift toward high-salience subtopics regardless of the stated debate question. A debate on "AI hiring tools in low-risk consumer products" can spiral into existential risk territory within three rounds if no mechanism constrains the argument space.

I implement a 5-mechanism topic-alignment pipeline that enforces debate scope without restricting argumentative creativity within scope.

**M1: TopicScope extraction.** Before the first debater turn, a dedicated LLM call extracts a structured TopicScope from the debate question — a 14-field schema comprising 7 universal fields (core_proposition, relevant_disciplines, on_scope_evidence, key_tensions, off_scope_topics, drift_signatures, example_ceiling) and 7 user-constraint fields (risk_level, domain, product_type, time_horizon, excluded_scenarios, explicit_qualifiers, constraint_confidence). The `constraint_confidence` field distinguishes `explicit` constraints (the user stated them) from `inferred` constraints (the LLM derived them from the question), enabling downstream mechanisms to calibrate enforcement strictness. Three worked examples in the extraction prompt demonstrate the mapping from natural-language debate questions to structured scope fields.

**M2: Prompt boundary placement.** The extracted scope block is injected at two high-attention positions in every debater prompt: immediately after the CHARACTER header (primacy position) and as a one-line reminder at the end of the recap section (recency position). This dual placement exploits the U-shaped attention curve (Liu et al., 2023) to ensure scope constraints occupy the positions where instruction-following is strongest.

**M3: Draft quality gate.** The existing draft quality check (Section 6.3, Stage B) receives a new TOPIC_ALIGNED question: the LLM judge assesses whether the draft response stays within the extracted scope. Failed topic alignment produces a specific repair hint directing the debater to refocus.

**M4: Taxonomy constraint-aware filter.** After embedding-based relevance scoring but before node injection, a deterministic post-selection filter adjusts node scores based on scope constraints. Three demotion layers (applied in priority order) penalize nodes with catastrophic framing in low-risk debates (2+ signals from a 14-word catastrophic vocabulary), nodes matching excluded scenarios (2+ term matches), and nodes matching off-scope topics (3+ term matches). A complementary discipline relevance boost (1.2x) promotes nodes whose content aligns with the scope's `relevant_disciplines` (2+ term matches). Demotion takes priority over boost — a node matching both catastrophic signals and discipline terms is demoted, never boosted. A per-BDI-category minimum floor (default 3) prevents over-filtering from starving any category, restoring the least-penalized demoted nodes when needed.

**M5: Moderator drift detection.** The active moderator's semantic drift detection (Section 6.22) is extended with two scope-aware patterns: risk-level mismatch (a debater introduces catastrophic framing in a low/medium-risk debate) and domain mismatch (a debater argues from a domain outside the scope's relevant disciplines). Both patterns trigger targeted REDIRECT interventions.

The five mechanisms operate at different pipeline stages — extraction (M1), prompt construction (M2), turn validation (M3), taxonomy selection (M4), and moderation (M5) — providing defense in depth against scope drift. Each mechanism degrades gracefully: if TopicScope extraction fails, the remaining mechanisms operate without scope constraints rather than blocking the debate.

### 6.25 Voice Differentiation

A persistent challenge in multi-agent debate is *voice homogeneity*: all agents sound like the same cautious, hedge-heavy, meta-commentary-prone AI assistant regardless of their assigned perspectives. This undermines the core premise of multi-perspective debate — if all voices are identical, the perspectives are distinguished only by content, not by the rhetorical character that makes each perspective's arguments persuasive to its natural audience.

I identify nine categories of AI voice tells through systematic analysis of debate transcripts:

| Category | Pattern | Example |
|----------|---------|---------|
| Hedge stacking | Multiple hedges in a single clause | "It could potentially perhaps be argued..." |
| False balance | Unprompted both-sidesing | "While there are valid points on both sides..." |
| Formulaic transitions | Stock AI transitional phrases | "Building on this foundation..." |
| Bullet-point reasoning | Enumerated lists instead of prose argumentation | "There are three key considerations: 1)..." |
| Sycophantic framing | Excessive validation of opponent positions | "That's an excellent and insightful point..." |
| Meta-commentary | Narrating one's own reasoning process | "Let me now turn to the question of..." |
| Synonym chains | Redundant near-synonym sequences | "robust, comprehensive, and thorough" |
| Theatrical emphasis | Melodramatic intensifiers | "This is absolutely critical and cannot be overstated..." |
| Passive voice overuse | Agency-obscuring constructions | "It has been suggested that concerns have been raised..." |

Each debate character receives per-character voice specifications comprising three components: `prose_style` (sentence structure, paragraph rhythm, rhetorical preferences), `rhetorical_arsenal` (preferred argument patterns and rhetorical devices), and `voice_hygiene` (character-specific prohibitions targeting the voice tells that character is most prone to). A shared VOICE AUTHENTICITY block injected into all character prompts provides universal anti-artifacting directives.

Preliminary evaluation shows 80-93% reduction in AI voice tell frequency on Claude Opus 4.6 across the nine categories, measured by automated detection of voice tell patterns in debate transcripts. However, the same voice specifications produce 0% improvement on Gemini 2.0 Flash Lite, indicating that voice differentiation effectiveness is strongly model-dependent — smaller models may lack the instruction-following capacity to suppress deeply ingrained generation patterns. This model-dependent finding has implications for multi-agent systems that deploy different models for different agents.

### 6.26 Epistemic Type Integration

Each taxonomy node carries an `epistemic_type` field classifying the kind of knowledge claim the position makes: strategic_recommendation (proposing a course of action), empirical_claim (asserting something observable), interpretive_lens (providing a framework for understanding), predictive (forecasting future states), normative_prescription (declaring what ought to be), or definitional (establishing what terms mean). These six types are orthogonal to BDI categories — a Belief can be empirical or predictive, a Desire can be normative or strategic.

Epistemic types are integrated into five debate engine components:

1. **Debater taxonomy context.** Each node's epistemic type is included in the injected context, with instructions to match argument mode to claim type: empirical claims demand evidence and measurement limitations; normative prescriptions demand named ethical frameworks and is/ought distinction; predictive claims demand timeframes and confidence intervals; strategic recommendations demand feasibility analysis; definitional claims demand term clarification before substantive engagement; interpretive lenses demand engagement with the framing itself, not just evidence within it.

2. **Moderator drift detection.** An EPISTEMIC TYPE & ASSUMPTION AWARENESS block detects cross-type talking-past-each-other: when one debater makes an empirical claim and the other responds with a normative prescription, the moderator intervenes to name the type mismatch and redirect the exchange to address both dimensions.

3. **Plan stage strategy.** The PLAN stage receives epistemic type metadata and instructs debaters to select argument strategies appropriate to their strongest nodes' types — building on a predictive node requires specifying a timeline, while building on a definitional node requires establishing terms first.

4. **Audience-weighted prioritization.** Different audiences weight epistemic types differently during taxonomy node selection: policymakers prioritize strategic recommendations (actionable outputs); technical researchers prioritize empirical claims (evidence-grounded analysis); the general public prioritizes normative prescriptions (value-driven accessibility). This integration connects the audience targeting system (Section 6.15) to the epistemic structure of the taxonomy.

5. **Draft quality gate.** The CALIBRATED dimension checks whether a debater's rhetoric matches the evidential strength of the empirical claims they cite — a low-confidence empirical claim cited as "established fact" fails; the same claim with appropriate hedging passes.

### 6.27 Emotional Register and Affect Appropriateness

Beyond propositional content, the register in which arguments are pressed carries diagnostic signal. Following Wachsmuth et al. (2017) on emotional appeal in argument quality, each speaker turn is scored across five affect dimensions — urgency, fear, hope, outrage, empathy — from curated lexicons. Two derived measures interpret the profile. *Affect intensity* is a weighted sum privileging the dimensions most capable of distorting deliberation (fear/outrage 0.30, urgency 0.20, hope/empathy 0.10). *Affect appropriateness* scores each turn against a phase baseline: confrontation admits higher urgency and outrage; argumentation shifts toward hope and empathy; the concluding phase is expected warmest and least inflammatory. A concluding turn pairing low appropriateness with high outrage raises an explicit warning — the debate is generating heat where it should consolidate. Scores are deterministic and, because each derives from the specific lexicon terms it matched, individually auditable; affect is computed only over debater turns (moderator/system entries excluded so procedural prose cannot contaminate the metric).
...
The Q-0 calibration outcome reveals a principled boundary between what current LLMs can and cannot reliably assess. Strategic claims (Intentions) are self-contained — their quality criteria (mechanism specification, failure mode awareness, implementation feasibility) are visible in the claim text and assessable through pattern recognition, the LLM's core strength. Normative claims (Desires) are also self-contained in principle, but under absolute-agreement statistics (ICC, Krippendorff's α), the AI assigns near-constant scores rather than discriminating between strong and weak normative arguments — self-containment is necessary but not sufficient for reliable automated scoring. Empirical claims (Beliefs) require external verification — checking source accuracy, evaluating evidence quality, and assessing consistency with the scientific record — which exceeds the model's reliable capabilities.
...
The hybrid approach (AI-calibrated for Intentions, AI-scored but not yet validated for Desires, human for Beliefs) is not a failure of prompt engineering — it is a principled acknowledgment of the current human-AI task allocation boundary. As LLMs gain better grounding in external knowledge (through retrieval augmentation, tool use, or training data improvement), the boundary may shift. But the BDI-aware rubric design will remain useful: even with better models, separating the assessment criteria by claim type produces more auditable scores than holistic judgment.

Our calibration signals are behavioral by construction: `crux_addressed_rate`, the claim-verification verdicts, and the op-ed grounding and fabrication guards all measure the *utterance*, not the model's internal state. Zou et al. (2023) make this limit precise. Their representation-engineering method locates the direction in a model's hidden-state space that encodes a concept such as truthfulness and reads behavior off that direction, and in doing so draws a distinction our output-only instruments cannot: *truthfulness* (outputs match the world) versus *honesty* (outputs match the model's own internal beliefs). Because a model can hold a consistent internal representation of truth while emitting a falsehood, no output-level signal can observe the gap between internal belief and utterance. We adopt this as a stated boundary on our scoring rather than a capability: any claim that our metrics detect debater "honesty" is a claim about outputs only. Their hidden-state method is not available on our stack — our production backends are API models exposing tokens and logprobs, not layer activations, so representation reading cannot run on them — and we flag this so the limitation is not overstated. The paper's contrast-stimulus design principle, that paired experimental-versus-reference framing increases separability, does transfer to our black-box setting and informs how we construct polarity and judge prompts.
...
A failure mode specific to systems that read from stores they also write to deserves separate treatment. Bronder (2026b) studies what he terms provenance laundering: a claim that was merely *written* in an earlier session returns wrapped in a store's retrieval envelope — a record id, a timestamp, a JSON field — and is thereafter read as an established, retrieved fact even though nothing new is known. Across three prospective studies on Claude Opus 5, an unsupported claim packaged as a retrieved record was adopted at high rates (14/24 in the exploratory arm, replicated at 7/24, one-sided Fisher p = 0.0047), while the identical claim presented as a bare prior assistant assertion was adopted at zero. Two of his findings bear directly on our design. First, marking a record "unchecked" did not reduce adoption — the annotated wrapper produced one *more* false adoption than the raw record — so a provenance flag in the data does not, by itself, stop a downstream model from relying on machine-authored content once that content is in the prompt. Second, announced inline text matched the retrieval envelope (60/60 vs 57/60), so the operative variable is not the tool channel but whether unsupported self-authored content is *framed* as an established fact and placed prominently. Our system has this shape in at least three places: situation injection re-enters pipeline- or LLM-generated context into a debate as authoritative background; machine-proposed evidence links (WS-B, ~0.63 precision) carry an `evidence_provenance` field that this result endorses as a safety control but warns is insufficient on its own; and taxonomy RAG retrieves partly LLM-authored node descriptions as "evidence." We therefore treat provenance as a safety control that travels *with* content into the prompt rather than as bookkeeping, hold machine-origin evidence links out of authoritative generative context without re-grounding, and frame injected and retrieved context as unverified candidates rather than as verified facts. The specific adoption rates do not transfer — one model, one synthetic task, one API — but the laundering shape and the provenance-as-control framing do.
...
Argument acceptability is computed symbolically rather than read from a language-model verdict. Each conflict is represented as an edge-weighted quantitative bipolar argumentation framework and resolved with DF-QuAD gradual semantics, giving every argument an acceptability degree in [0,1] whose inputs (base scores, typed attack and support edges, and their weights) are fully inspectable. A methodological caution follows directly from the semantics. An argument's acceptability is initialized at its intrinsic base score and is moved only by incident edges, so an argument that is never attacked retains its full base score and can rank alongside one that has withstood sustained challenge. Argument strength and argument testedness are therefore distinct dimensions that must not be conflated. Testedness is tracked separately through a Debate-Tested tier (untested, cited, contested, well-tested) keyed on whether an argument survived a severe attack, and both dimensions are surfaced in the diagnostics rather than collapsed into one score. The broader design principle is to place structured, auditable channels alongside the language-model verdict channel, so a claim's standing can be inspected rather than merely asserted.

### 8.8 Addressing the Fixed-Role Critique

A natural objection to the system's design is that three agents with permanent identities narrow the argument space. If Accelerationist always argues from the accelerationist taxonomy, Safetyist always from the safetyist taxonomy, and Skeptic always from the skeptic taxonomy, then arguments that cross these boundaries — compromise positions, shared assumptions, positions that no single POV would originate — will never surface through the agents alone. The critique is valid: the argument space of the debate system is bounded by the combined content of the three taxonomies.

However, the critique frequently misdiagnoses the cause. Proposals such as shadow debates (agents temporarily swap roles), role rotation (Accelerationist argues the safetyist position), and devil's advocate rounds (an agent argues against its own taxonomy) assume that agent identity and taxonomy are separable — that Accelerationist could meaningfully argue from Safetyist's taxonomy while remaining Accelerationist. In this system, this assumption is false. The taxonomy *is* the identity. Each agent's BDI-structured context injection (Section 3.3), its commitment store, its relevance scoring, its situation node interpretations, and its validated move history are all grounded in its assigned taxonomy. "Accelerationist argues the safetyist position" does not produce a novel perspective; it produces a second Safetyist with a confusing name.
...
The QBAF attack weights our conflict analysis carries (differentiated by attack type) are currently stipulated rather than validated against data. Gould and Toni (2025) demonstrate a concrete method for removing that stipulation: in their Gradual AA-CBR the edge weights of an edge-weighted QBAF are learned end to end by gradient descent under a differentiable gradual semantics, comparable in accuracy to plain neural networks while retaining case-grounded, inspectable structure. We do not adopt their supervised classifier wholesale — it requires a labelled casebase and trained feature extractors our LLM-debate pipeline does not have, and formalizing the whole system into trained QBAFs would run against our stated preference for ontological vocabulary over heavyweight formalism. What their result gives us is a named validation path: should a labelled conflict-resolution outcome set ever be built, gradient-learned edge weights convert our "someday validate these weights" note into a specific, published method, and a gradual-semantics acceptability degree computed over our typed argument graph offers an auditable, whiter-box complement to the LLM crux verdict, whose inputs, edges, and weights are all inspectable.
...
**Topic-alignment evaluation gap.** The 5-mechanism TopicScope pipeline is fully implemented but has not been formally evaluated. Controlled comparison (debates with vs. without scope enforcement on identical topics) is needed to measure off-topic drift reduction. The keyword-based taxonomy constraint filter (M4) may miss semantic drift that does not surface specific off-scope terms — embedding-based scope checking would improve coverage but adds computational cost.

**Voice differentiation model dependence.** The 80-93% de-artifacting reduction was measured only on Claude Opus 4.6. The 0% result on Gemini Flash Lite suggests a model-capacity threshold for effective voice specification that has not been characterized. Systematic evaluation across model families and sizes is needed to determine when voice differentiation becomes effective.
...
**Measurement-instrument effects.** A construct-validity concern shadows every claim drawn from debate transcripts: are we measuring the models' reasoning, or the shape our own instrument imposes on it? Bronder (2026a) makes this threat concrete and auditable in a single-system demonstration: holding the model fixed and varying only the measurement apparatus, the outcome taxonomy and the disclosed grading criterion moved measured "honesty" more than any model intervention. Expanding the verdict grammar from two categories to three collapsed "complete" verdicts from 22/40 to 7/40 and drove 28/40 runs into a newly reachable "incomplete," while disclosing the grading criterion eliminated false verdicts entirely (18/59 → 0/58). We take three of Bronder's four integrity checks as explicit constraints on our own measurement layer: (1) taxonomy saturation — our convergence and disagreement-type grammars are being extended to express a calibrated "crux-not-identified / not-yet-decidable" outcome, without which they can manufacture false strong verdicts; (2) criterion disclosure — the persona-free neutral evaluator's scoring rubric is specified in-prompt rather than left implicit (Section 6.9); and (3) distribution replication — quality metrics are computed over repeated runs rather than single transcripts. The fourth check, censoring analysis, we note as future work.
...
Studies are ranked by **inferential value** (how much a clean result changes what can be asserted in the paper) × **feasibility** (cost and design cleanliness) × **relevance** (how central the manipulated component is to the system's core claim).
...
The first three are the paper's spine. They share one topic set and one frozen model, manipulate the component the system is most identified with — engineered relevance — and two of the three are genuinely causal. If only three experiments run, run these. All studies fix the model (`ai-models.json`, one backend pinned for the whole study) and report effect sizes with bootstrap confidence intervals.

**1. Relevance-as-treatment on situation injection (causal).** *Hypothesis:* Injecting situations selected for high relevance raises `situation_crux_alignment` relative to injecting situations selected for low relevance, with model, prompt, topic, and injection count held fixed. *Manipulated component:* The relevance band of the injected set — high-relevance arm draws top-scoring situations; low-relevance arm draws situations scoring just above the floor, matched on count. This is the cleanest single-point intervention the system affords, because relevance is a continuous input the selector already computes. *Outcome metrics:* Primary: `situation_crux_alignment`. Secondary: `crux_addressed_ratio` and `avg_utilization_rate` (to confirm the high-relevance set is actually used and not merely present). *Sample:* 12 topics × 2 arms × 5 replications on fixed seeds = 120 runs, seeds paired within topic.

**2. Situation injection: none / random / relevance-ranked (ablation).** *Hypothesis:* A debate with relevance-ranked injection scores higher on `situation_crux_alignment` and `crux_addressed_ratio` than random injection, which in turn scores higher than none. *Manipulated component:* The situation-selection stage, set to three levels: off, random-sample-at-fixed-count, and the production relevance-ranked selector. The random arm is the load-bearing control — it separates "situations help" from "relevance-ranking helps," which the none-versus-production contrast alone cannot. *Outcome metrics:* `situation_crux_alignment`, `crux_addressed_ratio`, `situation_nodes_referenced` over `situation_nodes_injected`. *Sample:* 12 topics × 3 arms × 5 replications = 180 runs.

**3. Situation-count dose-response (causal).** *Hypothesis:* `convergence_score` rises with the number of injected situations up to a point, then falls as the context fragments — an inverted-U, not a monotone gain. This is the empirical test of the situation-cap parameter. *Manipulated component:* The injection count (`situation_max_nodes`), swept across levels: 0, 2, 5 (near current), 10, 20. Situations are added in relevance order so each higher level is a superset of the lower. *Outcome metrics:* Primary: `convergence_score`. Secondary: `repetition_rate`, `claims_forgotten_rate` (the fragmentation signature is convergence falling while claims-forgotten rises), and `relevance_score_variance`. The peak location is the recommended cap, reported with a confidence band. *Sample:* 10 topics × 5 levels × 5 replications = 250 runs.

**4. Three-POV interpretations versus single shared (ablation).** *Hypothesis:* Giving debate agents all three POV interpretations of each situation raises `situation_crux_alignment` and `convergence_score` relative to a single shared interpretation, because the disagreement is encoded in the interpretations themselves. A null here would suggest the three interpretations decorate the context without shaping the exchange, which bears directly on the DOLCE D&S design claim. *Outcome metrics:* `situation_crux_alignment`, `convergence_score`, `camp_insularity_rate`. *Sample:* 12 topics × 2 arms × 5 replications = 120 runs. Feasibility is medium because the single-shared arm requires a collapse function that fairly summarizes three readings without smuggling the disagreement back in.

**5. Prompt-only intervention, model held fixed (causal).** *Hypothesis:* Changing one prompt while holding the model and seed fixed moves the target metric, establishing that the prompt, not the model, carries the attributed effect. *Manipulated component:* One prompt variant toggled between two authored versions (e.g., the `disagreement_type` elicitation instruction). *Outcome metrics:* For `disagreement_type` elicitation, `crux_addressed_ratio` primary. Replication variance is reported prominently, because a small prompt effect swamped by model stochasticity is the expected failure mode and must be visible. *Sample:* 12 topics × 2 prompt versions × 8 replications = 192 runs.

**6. Injection ordering: boundaries versus middle (ablation).** *Hypothesis:* Placing the highest-relevance situations at the boundaries of the injection block raises `avg_primary_utilization` relative to placing them in the middle — testing the Lost-in-the-Middle mitigation the system already implements. *Manipulated component:* Ordering only; the same tokens rearranged. *Outcome metrics:* `avg_primary_utilization`, `situation_crux_alignment`. *Sample:* 10 topics × 3 arms × 5 replications = 150 runs. A null would say the ordering rule costs nothing but also buys nothing — worth knowing before defending it in print.

**7. Counterfactual crux removal (causal).** *Hypothesis:* Removing the single situation that a debate's crux depends on causes position drift and a rise in `claims_forgotten_rate`, relative to removing a matched non-crux situation. *Controls:* Identical injection set minus one item; the arms differ only in which single situation is dropped, matched on relevance. *Outcome metrics:* `claims_forgotten_rate` primary; position drift via `convergence_score` trajectory relative to full-injection baseline. *Sample:* 10 topics × 2 arms × 5 replications = 100 runs + 50 baseline runs. Feasibility is low because identifying the crux-bearing situation requires a labeling step that must be pinned before runs begin.

**8. BDI decomposition on versus off (ablation).** *Hypothesis:* Typing extracted nodes into Beliefs, Desires, and Intentions, and formatting context grouped by BDI layer, raises `crux_addressed_ratio` relative to a flat untyped node list. Feasibility is low because turning BDI off touches both the extraction pipeline and the context formatter, risking changes to more than one component. *Outcome metrics:* `crux_addressed_ratio`, `convergence_score`, `extraction_coverage_rate`. *Sample:* 12 topics × 2 arms × 5 replications = 120 runs plus engineering for a faithful flat-mode path.

**9. `disagreement_type` elicitation on versus off (ablation).** *Hypothesis:* Eliciting `disagreement_type` (definitional / interpretive / structural) for each situation and passing it to debaters raises `crux_addressed_ratio` relative to omitting it. This overlaps with Study 5's mechanism and can share its harness. *Outcome metrics:* `crux_addressed_ratio` primary, `situation_crux_alignment` secondary. *Sample:* 12 topics × 2 arms × 6 replications = 144 runs.

**10. AIF edge typing on versus off (ablation).** *Hypothesis:* Typing argumentation edges with the 8 canonical AIF types raises argument-structure quality relative to untyped edges. *Outcome metrics:* `local_sufficiency_mean`, `unsupported_claim_rate`, `peer_referencing_rate`. *Sample:* 10 topics × 2 arms × 5 replications = 100 runs.

**11. Genus-differentia descriptions versus plain (ablation).** *Hypothesis:* Node descriptions written in the genus-differentia form improve extraction quality relative to plain free-text descriptions. Lowest rank because the component is least central to the system's core argument, but the study is cheap and closes a gap a reviewer might raise. *Outcome metrics:* `extraction_coverage_rate`, `mean_extraction_confidence`, `entailment_pass_rate`, `low_confidence_claims_rate`. *Sample:* 8 document sets × 2 arms × 5 replications = 80 runs.
...
What the suite does not establish is that any of these metrics tracks a human's judgment of a better debate. Every outcome here is the system measuring itself. That gap is the province of the validation-instrument work — the single-rater criterion-validity studies, the affect instrument, the provenance register — and this suite and that work should be presented as two halves of one argument. The ablations show the components move the system's own signals; the validation studies show whether those signals mean anything to a person. Neither half is sufficient alone. Presented together they are a defensible claim that the architecture earns its complexity.
...
2. **Different claim types require different assessment infrastructure.** The Q-0 calibration outcome (AI succeeds on Intentions, shows rank correlation without absolute agreement on Desires, fails on Beliefs) reveals a fundamental asymmetry that extends the self-contained vs. externally-verifiable distinction — self-containment is necessary but not sufficient for reliable automated scoring. This finding generalizes beyond this system to any automated argumentation pipeline.
...
17. **Topic-alignment enforcement requires defense in depth.** No single mechanism suffices to keep multi-agent debates on topic. The 5-mechanism pipeline — extraction, prompt placement, validation, taxonomy filtering, and moderation — provides layered enforcement where each mechanism catches drift that others miss. The TopicScope schema's distinction between `explicit` and `inferred` constraints enables calibrated enforcement: user-stated boundaries are enforced strictly, while LLM-inferred scope receives softer treatment.

18. **Voice differentiation is strongly model-dependent.** Per-character voice specifications with 9-category anti-artifacting directives achieve 80-93% reduction in AI voice tells on Claude Opus 4.6 but 0% on Gemini Flash Lite. This finding implies that voice differentiation in multi-agent systems cannot be treated as a prompt-only problem — it requires model selection aware of instruction-following capacity for stylistic constraints, not just factual content.

19. **Epistemic type integration connects ontology to rhetoric.** Classifying taxonomy nodes by epistemic type (6 types orthogonal to BDI categories) and threading those types through debate prompts, moderation, and validation produces more epistemically appropriate argumentation — empirical claims are argued from evidence, normative prescriptions from ethical frameworks, predictions with timeframes. The moderator's cross-type drift detection catches a failure mode invisible to content-only analysis: debaters talking past each other by responding to an empirical claim with a normative prescription.

Future work includes formal FIRE evaluation (E1), scaled concession harvesting validation, cross-lingual extension, integration with retrieval-augmented generation to address the Beliefs scoring gap, systematic evaluation of metaphor reframing effectiveness across debate corpora, longitudinal analysis of neutral evaluator divergence patterns to identify which persona framings most frequently bias synthesis, empirical validation of the neural-symbolic architecture's auditability claims through user studies with policy analysts examining dialectic traces and convergence diagnostics, controlled evaluation of topic-alignment enforcement (E11) measuring off-topic drift reduction, and cross-model evaluation of voice differentiation (E12) identifying the instruction-following capacity threshold for effective de-artifacting.
...
Bronder, J. (2026a). Instrument effects in language-model honesty evaluation: An auditable single-system demonstration. *arXiv preprint arXiv:2607.14399*. https://arxiv.org/abs/2607.14399

Bronder, J. (2026b). Does a tool result carry more authority than plain text? Three prospective studies of false-claim adoption in a synthetic assignment task with Claude Opus 5. *arXiv preprint arXiv:2608.14992*. https://arxiv.org/abs/2608.14992
...
Gould, A. and Toni, F. (2025). Neuro-argumentative learning with case-based reasoning. *Proceedings of the 19th International Conference on Neurosymbolic Learning and Reasoning (NeSy 2025)*. arXiv preprint arXiv:2505.15742. https://arxiv.org/abs/2505.15742
...
Jin, Z., et al. (2026). ARGORA: Orchestrated Argumentation for Causally Grounded LLM Reasoning and Decision Making. *arXiv preprint arXiv:2601.21533*. https://arxiv.org/abs/2601.21533
...
Potyka, N. (2021). Interpreting neural networks as quantitative argumentation frameworks. *Proceedings of the AAAI Conference on Artificial Intelligence*, 35(7):6463–6470. https://doi.org/10.1609/aaai.v35i7.16801
