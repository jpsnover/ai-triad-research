...
export default {
  meta: {
    ...
    messages: {
      missingRecorder:
        'Catch block missing error recording. Use getGlobalRecorder()?.record({...}) (renderer) or log.*.warn/error() (server/main), or add /* telemetry — silent by design */ comment. Refactoring a catch into a helper? The record()/log call must stay LITERALLY in the catch — extract only the non-recording tail (ADR-003).',
    },
    schema: [],
  },
  create(context) {
