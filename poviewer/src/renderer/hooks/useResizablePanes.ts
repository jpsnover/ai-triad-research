...
const STORAGE_KEY = 'poviewer-pane-widths';
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      setWidths(prev => {
        ...
        if (handleIdx === 0) {
          ...
          let newLeft = Math.max(MIN_WIDTH_PCT, Math.min(mousePct, combinedWidth - MIN_WIDTH_PCT));
          next[0] = newLeft;
          next[1] = combinedWidth - newLeft;
        } else if (handleIdx === 1) {
          // Between pane 1 and pane 2
          const leftEdge = prev[0];
          const combinedWidth = prev[1] + prev[2];
          let newMiddle = Math.max(MIN_WIDTH_PCT, Math.min(mousePct - leftEdge, combinedWidth - MIN_WIDTH_PCT));
          next[1] = newMiddle;
