...
- 2026-07-29 — DebateUI (t/1915, p/83#4): `git commit -- <paths> -m "msg"` — `-m` and the message parsed as pathspecs after `--`. Self-corrected by reordering to `git commit -m "msg" -- <paths>`. **9th instance / 8 agents** (DebateUI's 2nd — also hit it at p/79#1 lineage). No further per-instance action; still self-correcting on git's own rejection, corrective-guidance emit still pending the #80 Part-3 fix.
