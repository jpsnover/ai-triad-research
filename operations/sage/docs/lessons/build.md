...
- **Mechanism (Docker p/217#6, t/2047#6):** the deeper trigger was a **FLOATING base tag** — `ai-triad-base` is built `FROM node:22-bookworm-slim` (floating minor), so a **routine base rebuild silently shipped a Node security patch (22.23.1 → 22.23.2)** bringing **undici 6.28.0 + a TLS session-reuse change**. That broke the app's global-`fetch()` GitHub-API data load → the `/tmp` cache **never populated** → `/healthz` readiness **503** → SIGTERM **crash-loop**. So the chain had **two unpinned layers**: a floating `node:` tag inside the base (silent patch on rebuild) AND the Dependabot bump delivering that changed base to the app (`0b33fc18`) — both "unpinned → silent runtime change," and the build-only gate saw neither. **Diagnostic tip:** with the local Docker daemon down, **diff the two builds' SBOM artifacts** for the exact package delta (found the 22.23.1→.2/undici delta without running the images).
...
5. **Compounding factor C (root trigger) — a FLOATING base tag makes every rebuild a silent, unpinned dependency bump.** `FROM node:22-bookworm-slim` (floating minor) pulls whatever patch is current at rebuild time (here 22.23.2 / undici TLS change). **Pin the base patch version in `Dockerfile.base`** (explicit, reviewable bumps) so a runtime change is a deliberate PR, not a side effect of a routine rebuild. Pairs with #1 (run-the-image smoke) — pinning makes changes explicit, the smoke catches the ones that still break.
6. **Diagnostic — daemon down? diff the SBOM artifacts.** When you can't run the images (local Docker daemon down), diff the two builds' **SBOM** artifacts for the exact package delta — surfaced the 22.23.1→.2 / undici 6.28.0 change without booting anything.
