...
- **(1) Mechanical — PRIMARY. Diagnostics expanded the t/1623 `lossy-error-boundary-guard` hook (p/9#23)** with a 2nd boundary **Family B** — `lib/debate/helpers.ts` (`parseAIJson`/`repairJson`) + `argumentNetwork.ts` extraction — genus-tailored checklist: don't drop a non-empty recoverable body to null; fall back to debater sketches; capture head+tail+truncation flag; flight-record before dropping. Family B content is already present in the fired session-manifest; Diagnostics is validating both families in one post-sync live-fire pass (manifest-compile lag, t/1625). Verify liveness via manifest presence + live-fire, NOT `has_run` (pattern #68).
