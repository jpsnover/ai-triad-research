...
**Status:** Active — **escalation triggered (5th instance, 2026-07-17).** 5 instances across t/1618–t/1621 + t/1626, **3 reporting agents** (TL, PowerShell, Diagnostics). The genus has **broadened** from the provider edge to *any generic recovery/parse boundary that discards a recoverable payload* (t/1626: `parseAIJson`→null in `lib/debate/helpers.ts` / `argumentNetwork.ts`) — which sits **OUTSIDE the current path scope** of the Diagnostics point-of-use hook (**t/1623** `lossy-error-boundary-guard`, scoped to `keys.ts` / `lib/ai-client/providers/*.ts` / `Test-AIApiKey` / `Invoke-AI*`). Escalation actions (p/8, p/9): (1) recommend Diagnostics **expand the t/1623 hook's watched paths** to cover the debate JSON-recovery sites (`parseAIJson`/`repairJson`, `argumentNetwork.ts` extraction); (2) flag to TL that the genus is escalation-triggered and ask whether a lib-level rule is warranted beyond ADR-001. The broadened hook (point-of-use) remains the lighter enforcement than a new prose `scripts/AGENTS.md` rule — ADR-001 already mandates ActionableError at boundaries. Backstop status: t/1623 hook authored + enabled (t/1623#1) but **not yet verified live** — verify via manifest presence + live-fire after a sync boundary, NOT `has_run` (pattern #68).
