...
- **Offender #5** (data-shape type-check, ≥4) → **t/1810 spike**, explicitly **NOT a blanket `isinstance` hook** — a false-red-prone check is a dead gate (#20/#46); the spike decides scoped-hook vs rule-only. This is the "candidate becomes a hook only if the trigger is cleanly detectable" prevention (#3) in action — resolve hookability by spike, don't ship a noisy guard.
- Class-total ≥12. **Sage standing action:** keep tagging new distinct offenders + both counters; watch t/1780 (In Review) and the t/1810 spike outcome.
