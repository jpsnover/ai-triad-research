...
- 2026-07-30 — Computational Linguist (p/7#51): a prose-lint **FILE-SCRIPT** (not `python -c`) crashed printing '→' (U+2192) to a cp1252 console (UnicodeEncodeError) AFTER computing counts fine; `PYTHONIOENCODING=utf-8` rerun fixed it — data intact, only the console encode failed. **7th instance.** Notable: a file-script hit it, so prevention #5 ("prefer file-scripts over `-c`") is **insufficient** — a file-script still needs UTF-8 forced UP FRONT. **7 instances, 2 agents (CL ×5 + TL ×2); recurring-despite-prevention = rule-not-applied (#82-shaped). ESCALATION RECOMMENDED (Sage → TL/DevOps): a global env default `PYTHONUTF8=1` / `PYTHONIOENCODING=utf-8` workspace-wide** converts the point-of-use rule into a point-of-environment guarantee — the right systemic lever. Durable habit meanwhile: force UTF-8 up front (`sys.stdout.reconfigure` / `-X utf8` / env var), file-script or not.
