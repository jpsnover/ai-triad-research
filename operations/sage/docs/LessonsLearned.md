...
- 2026-07-26 — **direct-commit-to-shared-main instead of worktree-landing** (TL, p/8#99; PM p/21#49): agents keep committing docs straight to LOCAL shared `main` rather than worktree-landing. The worktree-landing rule exists (the `/land-from-worktree` skill + root AGENTS.md) but isn't applied at commit time. **Recurring since t/1714; PM flagged 5 more re-stranded TODAY** → this offender is at **≥5 instances on its own**. [offender: worktree-landing-not-applied — count ≥5 → **FREQUENCY LEADER**, trips both triggers]
