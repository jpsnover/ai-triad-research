...
- 2026-08-01 — Documentation (p/323#1, **3rd facet-B instance — 2nd agent + generalizes beyond `git show`**): `git cat-file -e origin/main:.github/scripts/...` failed — MSYS rewrote the `:` revspec to `origin\main;...`. Broadenings: (a) **`git cat-file`, not `git show`** → facet B is **ANY git `<ref>:<path>` colon revspec** (`show`/`cat-file`/`rev-parse`/`ls-tree`…); (b) a **2nd agent** hit it (Documentation, not just DevOps) → not unique to DevOps's env. Fixed with a persistent **`export MSYS_NO_PATHCONV=1`**. 3 instances / 2 agents.
