...
- **Mechanism (Docker p/217#6, t/2047#6):** the deeper trigger was a **FLOATING base tag** — `ai-triad-base` is built `FROM node:22-bookworm-slim` (floating minor), so a **routine base rebuild silently shipped a Node security patch (22.23.1 → 22.23.2)** bringing **undici 6.28.0 + a TLS session-reuse change**. That broke the app's global-`fetch()` GitHub-API data load → the `/tmp` cache **never populated** → `/healthz` **503** → SIGTERM **crash-loop**. **Two unpinned layers**: a floating `node:` tag inside the base AND the Dependabot bump delivering it to the app (`0b33fc18`) — both "unpinned → silent runtime change," and the build-only gate saw neither. **Diagnostic:** daemon down → **diff the two builds' SBOM artifacts** for the exact package delta (found the 22.23.1→.2/undici delta without running the images).
