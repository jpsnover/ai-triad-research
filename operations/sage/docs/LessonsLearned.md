...
**Status:** Active — **BOTH triggers fired 2026-07-26; both offenders now have dispositions (TL, p/8#104):** #4 direct-commit-to-shared-main (≥5) → hook **already spec'd as t/1780** (In Review w/ Diagnostics, Gate-Verification + owner-go gated; the #5 data point reinforced priority). #5 data-shape type-check (≥4, CL p/7#36/#38 — reversed an earlier not-in-#82 call) → **t/1810 spike, NOT a blanket `isinstance` hook** (false-red-prone = dead gate, #20/#46); spike decides scoped-hook vs rule-only — prevention #3 in action. Class-total ≥12. Frame agreed p/8#94→#104. **Sage standing action:** keep tagging new distinct offenders + both counters; watch t/1780 + the t/1810 spike. (Sibling: direct-commit drove the large-divergence push failure p/9#36.)
