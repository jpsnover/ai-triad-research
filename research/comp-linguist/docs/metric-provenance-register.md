...
| Debate-Tested tier ladder (untested/cited/contested/well_tested + verdict rules) | stipulated | Implemented `lib/debate/debateTested.ts` (t/1545, `7f4b85f8`); renamed from "Corroboration" per t/1533. Validation study (reliability scope) at ≥50 recorded debates, weighted κ/α ≥ 0.7 target, frozen held-out sample. Two CL-review majors open (t/1575) before backfill. | — |
...
| `WELL_TESTED_MIN_CHALLENGES` (2) / `WELL_TESTED_MIN_DEBATES` (2) | stipulated | Owner expects to raise as corpus grows; recompute-only via `Update-NodeTestingRecord -RecomputeOnly` (Phase 2, t/1577). | — |
