...
1. **Extraction.** A batch LLM pass runs over the fact claims in `source_evidence_index.json` (a bounded corpus), via a config-driven UsageID (ADR-006). For each fact it proposes entities (name, type, alias candidates, supporting quote) with per-item confidence. Items below the gate are dropped; near-gate items are flagged. The gate is a stipulated threshold registered in the provenance register at PR time. The pass is **standalone in Phase 1** (run on demand, like the org-stance batches), with an optional later wiring into the ingestion pipeline so new documents trigger re-extraction; that wiring is a PowerShell-scope decision at Phase 3.
   **Person policy (owner decision, updated 2026-08-31):** `person` descriptions are **AI-drafted then human-edited**, superseding the original human-authored-only rule. The LLM may draft a person's description; the draft carries `description_provenance: ai-drafted` and stays `proposed`. A person record cannot be **approved** until a human edits the draft and sets `description_provenance` to `human-edited` (an `ai-drafted` or empty description blocks approval). Legacy person records with a non-empty description and no provenance field are grandfathered as `human-authored` (they provably predate LLM person-drafting). The human stays in the loop as editor, not sole author. Enforced by the gate in `Import-Entity` (t/3131); the `description_provenance` field is defined in `lib/entities/types.ts`.
...
export interface Entity {
  ...
  /** Genus-differentia: "A [type] that [differentia]...". AI-drafted then human-edited for `person` (see description_provenance). */
  description: string;
...
3. **Person descriptions are AI-drafted then human-edited** (updated 2026-08-31, superseding the original human-authored-only rule). The LLM may draft the description; approval requires a human to edit the draft and set `description_provenance: human-edited`. An `ai-drafted` or empty description blocks approval; legacy non-empty descriptions without a provenance field are grandfathered as `human-authored` (Section 4). Enforced in `Import-Entity` (t/3131).
