...
- **Phase 0 (spike validation, cheap).** Run the extraction prompt over the facts of roughly 20 nodes; hand-score yield and precision; confirm the excluded-class resolution catches org/pol/taxonomy collisions. Then exercise the **detect→link path end-to-end** on a few real debate statements (PM lifecycle review): alias-detect mentions in the statement text, resolve them to refs, and confirm each ref kind round-trips to the record a detail pane would need. This is the go/no-go before any schema ships.
