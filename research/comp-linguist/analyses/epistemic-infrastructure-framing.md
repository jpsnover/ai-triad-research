...
Everything below rests on one inference: a claim survived adversarial scrutiny. What survives a debate in this system is what survives scrutiny by other LLM outputs, under rules the project wrote, over a corpus the project curated, argued by perspectives the project authored. QBAF propagation computes coherence within that constructed network; nothing in the loop touches the world (modulo a fact checker based in web content). A claim can dominate the argument graph and be false. A claim can be annihilated in round two and be true. The scientific method, which appears in the lineage table at the end of this document, derives its authority from the one thing this loop lacks, an exogenous arbiter. Experiments touch reality. The reflect step touches the system's own prior outputs.

There are two mitigations. First, the loop's writes are human-gated. Harvested proposals do not change the taxonomy until a person reviews, edits, and accepts or rejects them (see Wisdom Harvesting). A human gate filters error; it does not convert survival into evidence of truth, because the gatekeeper is one researcher, not the world. Second, the system's most important product points beyond itself. A crux is by definition a question the loop cannot resolve internally, so identifying one marks where exogenous evidence, real experiments, and real institutional experience must be consulted. The loop's authority ends where the crux begins. The system clarifies the crux, it does not resolve it.
...
Some of these numbers do more than report; they steer. The system already tunes operational parameters from its own telemetry. When fewer than a third of injected taxonomy nodes get referenced, the admission threshold rises; when primary nodes go underused, it falls. The hypothesis behind this feedback loop is narrow. Usage signals sit on a different causal path from the knobs they tune, so a control loop over them optimizes operational fit without touching the measurement itself. The approach follows from the hypothesis. Operational knobs may self-tune against usage signals; instrument definitions (lexicons, distortion weights, quality-score formulas) may not be tuned against scores computed from themselves, because that moves the ruler to fit the measurement. Quality-score-driven adjustments stay recommendations until they pass a frozen golden set and leave untouched a set of sentinel metrics excluded from every tuning objective. Self-tuning improves how well the machine runs. It never upgrades a number's epistemic status.
...
Each of the three perspectives (accelerationist, safetyist, skeptic) is a complete BDI worldview, with structured beliefs about how AI works, desires about what society should prioritize, and intentions about how to get there. The same phenomenon is salient to each for different reasons. "AI governance" is an innovation bottleneck to the accelerationist, essential gating to the safetyist, a capture risk to the skeptic. Situation nodes hold all three interpretations simultaneously, each decomposed into its own belief, desire, and intention; the decomposition is corpus-wide (all 441 non-deprecated situations). They do not resolve which interpretation is correct. They make visible what each perspective foregrounds and backgrounds. Translating between salience frames is the Rosetta Stone function.
...
Structural conditions are necessary but not sufficient; a debate with all the right components can still degenerate into position-recycling, network-flooding, or tactical concession. A game-theoretic layer diagnoses this without turning agents into utility maximizers. Each agent carries a per-turn utility triple: position strength (mean relevance (dialectical) of its surviving nodes), attack effectiveness (opponent nodes weakened below viability), and crux engagement (identified cruxes directly addressed). The ratios reveal posture. High attack effectiveness with low crux engagement is scoring rhetorical points without advancing understanding; rising position strength with falling crux engagement is winning by avoidance, the most corrosive failure mode. Plotted across rounds, the curves expose stagnation, runaway dominance, and disengagement.

Concessions are deliberately left out of the utility score. If conceding cost utility, the lookahead gate and the agents it steers would learn that the winning move is to defend everything, and the debate would reward stubbornness over honesty. Conceding a point an opponent has earned is the most valuable epistemic act a debater can perform, so the utility function treats it as free, and the process-reward model goes further and credits it. What the layer watches instead is the concession pattern. A separate asymmetry signal compares the strength of what an agent attacks against the strength of what it concedes, weighting crux concessions more heavily, so strategic concession (giving away cheap, weak nodes to buy credibility while extracting valuable ones) surfaces as a diagnostic without ever making honest concession expensive.

Three anti-exploit patterns (filibustering, dialectical drift, preference faking) are detected and surfaced as calibration signals rather than hard blocks; the asymmetry signal is the preference-faking detector. The layer exists to hold the debate in the productive zone between too adversarial, which generates heat, and too cooperative, which generates false consensus.
...
### Debate-Tested: A Fourth Axis, History Rather Than Property

Confidence, priority, and operationality all describe a claim's inherent standing, how plausible, how important, how actionable, judged independent of whether the claim has ever actually been argued. A different question went unanswered. How much adversarial pressure has this specific claim, in its current wording, actually survived? A newly approved instrument, not yet live, answers it by rolling per-debate outcomes (held, weakened, refined, open, cited) into a small discrete tier, Untested, Cited, Contested, Well-Tested, rather than a continuous score, on the same false-precision grounds that keep affect and clarity from reporting decimals nobody could defend. The tier is Beliefs-only in its first version, a direct application of the BDI section's own claim that different kinds are contestable in different ways; an evidential attack, a feasibility challenge, and no attack surface at all are not the same construct, and forcing Desires and Intentions through one severity vocabulary would have measured three different things under a single badge.

The design borrows its central insight from Karl Popper, for whom a claim earns standing not by being plausible on its face but by surviving severe attempts at refutation, and its treatment of revision from Imre Lakatos, for whom a claim revised in response to a real challenge and subsequently retested holding is not a reset of standing but the strongest certificate a system can issue. The instrument was originally named Corroboration, after Popper's term, and external review found the name untenable on Popper's own terms rather than merely on readability grounds. Popper's account explicitly forbids treating corroboration as license to test a claim less, since a well-corroborated claim is not thereby more likely to survive its next test, and an early draft used the top tier to reduce a claim's future debate exposure under that same name. The mechanism survives the review, debate attention is finite and spending it where it teaches the most is defensible on ordinary resource-allocation grounds alone, but the borrowed name does not, and the instrument now ships as Debate-Tested, with Popper and Lakatos retained as cited inspiration rather than claimed equivalence.

The correction belongs in this document because it is the vocabulary-problem section's discipline turning inward. A high-stakes term borrowed from a real intellectual tradition cannot be reused for a mechanism that tradition would reject, and the system had to catch itself doing to its own design vocabulary what the controlled vocabulary exists to stop debaters from doing to contested policy terms. The instrument remains stipulated in the sense the register defines, every threshold a design-time choice. A validation study against human raters, at least a third unfamiliar with the project's own vocabulary, is specified but not yet run, waiting on enough debates to accumulate testing records worth sampling.

### Situations as Topic Generators

A calibrated debate engine running on a poorly chosen topic produces activity, not insight, so the taxonomy generates its own agenda. Every situation carries a `disagreement_type` naming why the perspectives diverge: definitional (different meanings for the same term), interpretive (shared observations, divergent conclusions), or structural (disagreement about institutional design). The type selects the debate strategy. Definitional disagreements suit Socratic definition-forcing; interpretive ones need simultaneous three-way engagement; structural ones yield the richest convergence under deliberation protocols. A divergence score computed from interpretation distance, BDI layer distribution, conflict saturation, and debate coverage ranks every situation by expected productivity, closing the loop into topic selection. Situations point to productive disagreements, debates test them, and the results feed back into the scoring. During setup, dominant intellectual-lineage clusters (11 families, 55 clusters over ~4,769 lineage references) act as a tiebreaker that surfaces tradition-relevant nodes without overriding relevance (topical).

### The SRE Mindset: Assume Fallibility And QC/QA Everywhere
...
### Organizations: Where the Loop Meets Named Reality

Everything described so far tests coherence inside a corpus the project curated, argued by perspectives the project authored. Organizations are the one place the system tracks something that exists independent of any debate, named real-world actors (laboratories, regulators, think tanks, advocacy groups) and how each aligns with the three camps. Until recently that alignment was a bare decimal, undefined in the strict sense the register elsewhere warns against; nothing anchored why one organization sat at +0.4 and another at +0.3, and the number silently conflated two different things, what an organization says and what it does. Anthropic is the case that makes the conflation visible, an organization whose public rhetoric and documented behavior diverge enough that a single score cannot honestly represent both.

The current work replaces the guess with a pipeline. Real source documents about a real organization are mined for stance claims, each claim is proposed as a candidate edge to a specific taxonomy node with its evidence attached, a human reviews every proposal through the same approve-or-reject gate the rest of the taxonomy uses, and only the approved edges feed a derived camp-alignment score. The rhetorical and behavioral questions are kept apart rather than merged back into one number. The derived pipeline measures rhetorical stance, what the organization has said; behavioral stance stays a human-curated annotation. The gap between the two is itself a finding worth preserving, not noise to average away.

What the pipeline has produced so far is worth reporting honestly rather than smoothing over. Every organization's derived score currently reflects zero usable evidence, either because the extracted candidate edges await human review or because, among the edges reviewed, the corpus has so far produced supporting claims and no opposing ones, an artifact of what the source documents happen to assert rather than a defect in the formula. A five-point tier ladder anchored to that distribution was specified as a follow-on piece of work and is deliberately not proceeding; anchoring boundaries to a distribution that is either entirely absent or uniformly one-sided would be re-guessing under a derived label, the exact promotion the register's no-grade-inflation rule exists to block. The pipeline stays in this holding pattern until a human closes the review queue or the corpus grows to include documents where an organization's position is genuinely contested, whichever comes first. Recording the block rather than working around it is the same discipline Survival Is Not Truth asks of every number in the system, applied to the one part of the system that reaches past its own debates toward actors who existed before it and will keep existing regardless of what it concludes about them.
