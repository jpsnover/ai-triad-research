...
The first two obligations are now tracked work, not gestures. **Affect-instrument validation** (t/1342): ~100 stratified debater turns human-rated on all five dimensions, agreement measured per dimension with ICC(2,1) and Krippendorff's α — the same absolute-agreement statistics that exposed the Desires calibration artifact — under a pre-registered decision rule: dimensions below α 0.4 are retired from affect intensity until their lexicons are revised, and the stipulated distortion weights are replaced with regression-derived ones or dropped. **A metric provenance register** (t/1343): every scalar in the calibration system classified as *stipulated*, *derived*, or *human-validated*, each with an evidence pointer, with the rule that anything lacking evidence is stipulated by definition — no grade inflation — and every new metric must declare its class at review time. The register makes false precision a visible, countable property of the system instead of a critique waiting to be written.
