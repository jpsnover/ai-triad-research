...
| 12 | Max-iteration-cap sweep | Ablation | Medium | High | Medium |

Study 12 is a later addition (t/1675) adapting Du et al. 2023's round-count ablation; by inferential value it slots alongside the mid-pack ablations (studies 5 and 9), but it is listed last as the newest entry and carries the highest engineering feasibility in the suite — its manipulation is a single config integer, with the analytical work sitting in a censoring-aware read rather than any new engine code.
...
### 12. Max-iteration-cap sweep (ablation)

**Hypothesis.** Raising the max-iteration cap increases crux engagement (`crux_addressed_ratio`) up to a knee, past which added rounds buy repetition (`repetition_rate` rises) rather than engagement. The quantity of interest is the cap beyond which more iteration stops purchasing crux engagement and starts purchasing circling. This adapts Du et al. 2023's round-count sweep — which reports monotone-with-diminishing-returns accuracy gains — to our process metrics, where the expectation is not a monotone gain but a point of first diminishing and then negative return, because our objective is engaged disagreement rather than a converged answer.

**Manipulated component.** The max-iteration cap `maxTotalRounds` (the debate engine's round-count ceiling; a CL-owned algorithmic parameter alongside temperature, convergence thresholds, and the situation cap). It surfaces in telemetry as `termination_reason='max_iterations'` when a run exhausts it. Swept across levels spanning below and above the production default — for example {3, 5, 8, 12, 20}, with the current default included as one level. Rounds accumulate in sequence, so up to the point a run concludes naturally each higher cap is a temporal continuation of the same trajectory: a run that reaches `natural_conclusion` at round 4 is identical under every cap ≥ 4.

**Controls.** Same topics, model, seed schedule, situation set and injection count, relevance-ranked selection, temperature, and ordering; only the cap changes. This is a single-integer manipulation with no new generation logic, which is what makes its engineering feasibility the highest in the suite.

**Outcome metrics.** Primary: `crux_addressed_ratio` (does more iteration raise crux engagement?) read against `repetition_rate` (or does it merely circle?). Secondary: `claims_forgotten_rate` (the long-run degradation signature; this is the metric Du et al.'s context-summarization trick would bear on, worth remembering if it regresses at the high-cap end) and the `convergence_score_at_termination` trajectory.

**The censoring confound is load-bearing here, not a footnote.** In this study the manipulated variable *is* the censoring lever. `censored` is defined as `termination_reason ∈ {max_iterations, situation_cap, api_ceiling}`; a low cap forces `max_iterations` termination on runs that would otherwise have reached `natural_conclusion`, and a high cap lets those same runs continue to a decision point. So the censoring rate changes across arms by construction. Two consequences the design must honor. First, the `censored` fraction per cap level is a first-class reported outcome, not a nuisance term — it is part of what the sweep measures. Second, the convergence outcome must be read **un-pooled**, over decision-point-reached (natural-conclusion) runs only, via `computeConvergenceWithCensoring`; a raw pooled `convergence_score_at_termination` compared across cap levels is confounded by differential censoring and must not be reported as the headline (t/1671, R-4). This makes the max-iteration sweep the one study in the suite where the treatment and the censoring trigger are the same knob, and its analysis is written accordingly.

**Expected effect and direction.** `crux_addressed_ratio` rises with the cap and then plateaus; `repetition_rate` runs flat-to-rising through the plateau and accelerates past it; `claims_forgotten_rate` climbs at the high-cap end; `censored` falls as the cap rises. The recommended cap is the knee where crux engagement has plateaued but repetition has not yet taken off, reported with a bootstrap confidence band. A finding that `crux_addressed_ratio` never plateaus — that more rounds keep buying engagement — is itself worth reporting, since it would argue the current cap is set too low.

**Persona count is not the other axis.** Du et al. sweep two knobs, round count and agent count, and report gains on both. We adopt the first as this study and explicitly reject the second as a scaling knob. Our agents are not interchangeable instances; the three personas (accelerationist, safetyist, skeptic) encode a fixed DOLCE/BDI-grounded ontology, so adding a fourth is an ontology change carrying its own genus-differentia justification burden, not a dial to turn. Varying persona count would confound any round-count effect with an ontology change and is out of scope for this ablation. If persona count is ever varied it must be studied as a design change with its own justification, never as a knob turn — the position the Du et al. relevance assessment already records (t/1610).

**Sample and seed plan.** 10 topics × 5 cap levels × 5 replications = 250 runs, seeds paired within topic across levels. The within-topic-seed pairing is especially tight here: because each higher cap continues the same seeded trajectory until natural termination, a topic-seed's lower-cap run is a literal prefix of its higher-cap run up to the point the lower cap cut it off.

**Rough cost.** 250 runs, but the cost is skewed toward the high-cap arm more than the run count implies: a 20-round debate is materially longer in tokens and wall-clock than a 5-round one, so the high-cap levels dominate the budget. Feasibility is nonetheless high because the manipulation itself is a config integer, with the censoring-aware read as the real work rather than any new engine code.

**Provenance.** Adding this study to the design defines and changes no metric, threshold, weight, or lexicon, so it triggers no `metric-provenance-register.md` entry now. If the sweep recommends a new `maxTotalRounds` default, that recommendation carries its own provenance declaration at that time — a cap chosen from this sweep's process-metric knee is `derived` — with a register entry (t/1675).

## What the suite does and does not establish

Run in full, this suite lets the paper say that relevance-ranked situation injection raises crux alignment over random and none (studies 1, 2), the effect is caused by relevance and not mere presence (study 1), situation count has an inverted-U optimum that fixes the cap parameter empirically (study 3), and the three-POV encoding, the ordering rule, the crux dependency, BDI typing, the argumentation and extraction scaffolds, and the max-iteration cap each carry their claimed weight or do not (studies 4–12). Two of the top three are causal; the rest are controlled ablations, labeled as such.
...
The recommended minimum for a first paper is studies 1, 2, and 3, sharing one topic set and one frozen model across 550 runs, which carry the system's central relevance claim with a causal result at the center. The remaining nine are the natural extension once the spine holds.
