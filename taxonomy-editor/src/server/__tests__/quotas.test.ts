...
vi.mock('../runtimeConfig.js', () => ({
  getConfig: vi.fn().mockReturnValue({
    quotas: { defaultMaxChats: 25, defaultMaxDebates: 15, defaultMaxOpEds: 15, defaultMaxBriefExports: 25 },
...
describe('quotas config load — fd path (t/2023)', () => {
  ...
  it('falls back to defaults when the config file is missing (openSync throws)', async () => {
    ...
    expect(limits.maxBriefExports).toBe(25);
...
describe('checkQuota', () => {
  ...
  it('allows brief-exports under the limit', async () => {
    const { checkQuota } = await loadModule();
    const result = checkQuota('brief-exports', 10, 'testuser');
    expect(result).toMatchObject({ allowed: true, current: 10, limit: 25, resource: 'brief-exports' });
  });

  it('blocks when at the brief-exports limit', async () => {
    const { checkQuota } = await loadModule();
    const result = checkQuota('brief-exports', 25, 'testuser');
    expect(result).toMatchObject({ allowed: false, current: 25, limit: 25 });
  });
});
