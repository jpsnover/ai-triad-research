...
// + deriveKeyErrorMessage) now live in ai/providerErrors.ts — a shared module once a
// 2nd call site (aiBackends.refreshAIModels) needed the identical mapping (Shared
// Utility Rule). Import for local use in validateProviderKey below AND re-export so
// keysValidation.test.ts keeps importing them from routes/keys.js unchanged. (A bare
// `export … from` re-export creates NO local binding — validateProviderKey's calls
// would throw ReferenceError, get swallowed by the silent catch, and yield reason:null.)
import { extractProviderReason, deriveKeyErrorMessage } from '../ai/providerErrors.js';
export { extractProviderReason, deriveKeyErrorMessage };
